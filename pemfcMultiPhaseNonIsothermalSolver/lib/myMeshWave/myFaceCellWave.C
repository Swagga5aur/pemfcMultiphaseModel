...
#include "processorPolyPatch.H" // OpenFOAM src file
#include "cyclicPolyPatch.H" // OpenFOAM src file
#include "OPstream.H" // OpenFOAM src file
#include "IPstream.H" // OpenFOAM src file
#include "PstreamReduceOps.H" // OpenFOAM src file
#include "debug.H" // OpenFOAM src file
...
const Foam::scalar Foam::myFaceCellWave<Type, TrackingData>::geomTol_ = 1e-6;

template <class Type, class TrackingData>
const Foam::scalar Foam::myFaceCellWave<Type, TrackingData>::propagationTol_ = 0.01;

// added (J. P. Kone, 09.09.2016)
template<class Type, class TrackingData>
int Foam::myFaceCellWave<Type, TrackingData>::dummyTrackData_ = 12345;

// Write to ostream
template <class Type, class TrackingData>
Foam::Ostream& Foam::myFaceCellWave<Type, TrackingData>::writeFaces
(
    const label nFaces,
...
// Read from istream
template <class Type, class TrackingData>
Foam::Istream& Foam::myFaceCellWave<Type, TrackingData>::readFaces
(
    label& nFaces,
...
template <class Type, class TrackingData>
bool Foam::myFaceCellWave<Type, TrackingData>::updateCell
(
    const label cellI,
...
template <class Type, class TrackingData>
bool Foam::myFaceCellWave<Type, TrackingData>::updateFace
(
    const label faceI,
...
template <class Type, class TrackingData>
void Foam::myFaceCellWave<Type, TrackingData>::checkCyclic
(
...
) const
{
    ...
    for(label patchFaceI = 0; patchFaceI < cycOffset; patchFaceI++)
    {
        ...
        if (!allFaceInfo_[i1].sameGeometry(mesh_, allFaceInfo_[i2], geomTol_, td_))
        {
            FatalErrorIn("myFaceCellWave<Type>::checkCyclic(const polyPatch&)")
...
template <class Type, class TrackingData>
bool Foam::myFaceCellWave<Type, TrackingData>::hasPatchType(const word& nameOfType)
{
...
template <class Type, class TrackingData>
void Foam::myFaceCellWave<Type, TrackingData>::setFaceInfo
(
...
template <class Type, class TrackingData>
void Foam::myFaceCellWave<Type, TrackingData>::mergeFaceInfo
(
    const polyPatch& patch,
    const label nFaces,
    const labelList& changedFaces,
...
template <class Type, class TrackingData>
Foam::label Foam::myFaceCellWave<Type, TrackingData>::getChangedPatchFaces
(
    const polyPatch& patch,
...
template <class Type, class TrackingData>
void Foam::myFaceCellWave<Type, TrackingData>::leaveDomain
(
    const polyPatch& patch,
    const label nFaces,
    const labelList& faceLabels,
...
template <class Type, class TrackingData>
void Foam::myFaceCellWave<Type, TrackingData>::enterDomain
(
    const polyPatch& patch,
    const label nFaces,
    const labelList& faceLabels,
...
template <class Type, class TrackingData>
void Foam::myFaceCellWave<Type, TrackingData>::transform
(
...
template <class Type, class TrackingData>
void Foam::myFaceCellWave<Type, TrackingData>::sendPatchInfo
(
...
template <class Type, class TrackingData>
Foam::label Foam::myFaceCellWave<Type, TrackingData>::receivePatchInfo
(
    const label neighbour,
    labelList& faceLabels,
    List<Type>& faceInfo
...
template <class Type, class TrackingData>
void Foam::myFaceCellWave<Type, TrackingData>::offset
(
    const polyPatch&,
...
template <class Type, class TrackingData>
void Foam::myFaceCellWave<Type, TrackingData>::handleProcPatches()
{
    // Send all
...
template <class Type, class TrackingData>
void Foam::myFaceCellWave<Type, TrackingData>::handleCyclicPatches()
{
    forAll(mesh_.boundaryMesh(), patchI)
    {
        const polyPatch& patch = mesh_.boundaryMesh()[patchI];
...
template <class Type, class TrackingData>
Foam::myFaceCellWave<Type, TrackingData>::myFaceCellWave
(
    const polyMesh& mesh,
    ...
    iter_(0)
{
    ...
    if ((maxIter > 0) && (iter_ >= maxIter))
    {
        FatalErrorIn
        (
            "myFaceCellWave<Type>::myFaceCellWave"
...
template <class Type, class TrackingData>
Foam::label Foam::myFaceCellWave<Type, TrackingData>::getUnsetCells() const
{
...
template <class Type, class TrackingData>
Foam::label Foam::myFaceCellWave<Type, TrackingData>::getUnsetFaces() const
{
...
template <class Type, class TrackingData>
Foam::label Foam::myFaceCellWave<Type, TrackingData>::faceToCell()
{
    ...
    )
    {
        ...
        if (!changedFace_[faceI])
        {
            FatalErrorIn("myFaceCellWave<Type>::faceToCell()")
                << "Face " << faceI
...
// Propagate cell to face
template <class Type, class TrackingData>
Foam::label Foam::myFaceCellWave<Type, TrackingData>::cellToFace()
{
    ...
    )
    {
        ...
        if (!changedCell_[cellI])
        {
            FatalErrorIn("myFaceCellWave<Type>::cellToFace()")
                << "Cell " << cellI
                << " not marked as having been changed"
                << abort(FatalError);
        }
...
// Iterate
template <class Type, class TrackingData>
Foam::label Foam::myFaceCellWave<Type, TrackingData>::iterate(const label maxIter)
{
    if (hasCyclicPatches_)
    {
        // Transfer changed faces across cyclic halves
