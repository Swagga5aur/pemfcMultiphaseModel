...
#include "fvMesh.H" // openfoam src file, added for FatalIOErrorIn (J. P. Kone, 11.09.2016)

// * * * * * * * * * * * * * * Static Data Members * * * * * * * * * * * * * //

namespace Foam
{
    defineTypeNameAndDebug(myPorosityModel, 0);
    defineRunTimeSelectionTable(myPorosityModel, mesh);
}


// * * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * //

void Foam::myPorosityModel::adjustNegativeResistance(dimensionedVector& resist)
{
...
Foam::label Foam::myPorosityModel::fieldIndex(const label i) const
{
    label index = 0;
...
Foam::myPorosityModel::myPorosityModel
(
    const word& name,
...
void Foam::myPorosityModel::transformModelData()
{
...
Foam::tmp<Foam::vectorField> Foam::myPorosityModel::myPorosityModel::force
(
...
void Foam::myPorosityModel::addResistance(fvVectorMatrix& UEqn)
{
    if (cellZoneIDs_.empty())
    {
        return;
...
void Foam::myPorosityModel::addResistance
(
    fvVectorMatrix& UEqn,
    const volScalarField& rho,
    const volScalarField& mu
...
bool Foam::myPorosityModel::writeData(Ostream& os) const
{
    return true;
}


bool Foam::myPorosityModel::read(const dictionary& dict)
{
